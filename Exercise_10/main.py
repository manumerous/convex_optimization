...
def compute_separate_classifier_results(pokemoves_matrix, poketype_matrix,  poketype_list, n_train):
    ...
    m_mat_test = m_mat[:, range(n_train, m_mat.shape[1])]
    test_poketype_matrix = poketype_matrix.iloc[range(n_train, 801), :]
    print(test_poketype_matrix.shape)

    single_poke_id_list = []
    double_poke_id_list = []
    for i in range(test_poketype_matrix.shape[0]):
        cur_row = test_poketype_matrix.iloc[i, :].to_numpy()
        if (cur_row.sum() == 1):
    ...
    print(m_mat_test.shape)
    m_mat_single = m_mat_test[:, single_poke_id_list]
    m_mat_double = m_mat_test[:, double_poke_id_list]

    poketype_single = test_poketype_matrix.iloc[single_poke_id_list, :]
    poketype_double = test_poketype_matrix.iloc[double_poke_id_list, :]

    test_classifier_single = booleanize_classifier(m_mat_single.T @ beta_mat)
    classifier_results_single = evaluate_classifier(test_classifier_single, poketype_single.to_numpy(), poketype_list)

    test_classifier_double = booleanize_classifier(m_mat_double.T @ beta_mat)
    classifier_results_double = evaluate_classifier(test_classifier_double, poketype_double.to_numpy(), poketype_list)
    return classifier_results_single, classifier_results_double
...
def main():
    ...
    # poketype_matrix = poketype_matrix.iloc[:,range(1,19)]
    # pokemoves_matrix = pokemoves_matrix.iloc[:,range(1,802)]
    # print(poketype_matrix)
    # print(pokemoves_matrix)
    ...
    # single beta for both sets:
    classifier_results_single, classifier_results_double = compute_separate_classifier_results(pokemoves_matrix, poketype_matrix,  poketype_list, 400)
    classifier_results_single = add_balanced_accuracy(classifier_results_single)
    classifier_results_double = add_balanced_accuracy(classifier_results_double)

    print(classifier_results_single)
    print(classifier_results_double)

    classifier_results_single.to_csv('single_classifier_single_beta_results.csv')
    classifier_results_double.to_csv('double_classifier_single_beat results.csv')
    ...
    single_classifier_results.to_csv('single_classifier_results.csv')
    double_classifier_results.to_csv('double_classifier_results.csv')
